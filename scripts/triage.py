...
def check_xmllint_2017_9049(buf):
    if "heap-buffer-overflow" in buf:
        if "dict.c:448:" in buf:
            return True
        elif get_crash_func(buf) == "xmlDictComputeFastKey":
            warn("Unexpected crash point in xmlDictComputeFastKey", buf)
    return False
